...
BlurNodeSampling::BlurNodeSampling(void)
	: _textureBall(NULL),
...
bool BlurNodeSampling::init()
{
	if (!Node::init())
		return false;
	// shader
	_shaderHorizontal = GLProgram::createWithFilenames("ccShader_blur.vert", "Blur_horizontal_sampling.frag");
	if (!_shaderHorizontal)
		return false;
	_shaderHorizontal->retain();
	_shaderVertical = GLProgram::createWithFilenames("ccShader_blur.vert", "Blur_vertical_sampling.frag");
	if (!_shaderVertical)
		return false;
...
BlurNodeSampling* BlurNodeSampling::create()
{
	BlurNodeSampling* result = new BlurNodeSampling();
...
void BlurNodeSampling::update( float delta )
{
	CCLOG("Time: %f", delta);
...
void BlurNodeSampling::draw( Renderer *renderer, const Mat4& transform, uint32_t flags )
{
